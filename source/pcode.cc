...
constexpr int STACK_BITS = 20, HEAP_BITS = 30;
constexpr int STACK_SIZE = (1 << STACK_BITS);
...
class Allocator
{
  ...
  public:
    Allocator() : free_list(HEAP_BITS + 1)
    {
        free_list[HEAP_BITS].push_back(0);
        alloc(STACK_SIZE);
    }

    int alloc(int size)
    {
        if (size <= 0 || size > (1 << HEAP_BITS))
            return 0;
        int k_req = 0;
        while ((1 << k_req) < size)
            ++k_req;
        int j = k_req;
        while (j <= HEAP_BITS && free_list[j].empty())
            ++j;
        if (j > HEAP_BITS)
            return 0;
        int blk = free_list[j].back();
        free_list[j].pop_back();
        for (int cur = j; cur > k_req; --cur)
        {
            int left = blk << 1;
            int right = left + 1;
            free_list[cur - 1].push_back(right);
            blk = left;
        }
        int addr = blk << k_req;
        allocated_map[addr] = k_req;
        return addr;
    }
    ...
    void free(int addr)
    {
        if (addr == 0)
            return;
        auto it = allocated_map.find(addr);
        if (it == allocated_map.end())
            return;
        int k = it->second;
        allocated_map.erase(it);
        int blk = addr >> k;
        while (k < HEAP_BITS)
        {
            int buddy = blk ^ 1;
            auto pos = std::find(free_list[k].begin(),
                                 free_list[k].end(), buddy);
            if (pos == free_list[k].end())
                break;
            free_list[k].erase(pos);
            blk >>= 1;
            ++k;
        }
        free_list[k].push_back(blk);
    }

  private:
    std::vector<std::vector<int>> free_list;
    std::unordered_map<int, int> allocated_map;
};
...
std::map<int, long long> heap_mem;
Allocator allocator;
...
long long mem_load(int addr)
{
    if (addr < 0)
    {
        std::cerr << "Runtime error: negative memory access"
                  << std::endl;
        std::exit(1);
    }
    if (addr < STACK_SIZE)
...
void mem_store(int addr, long long val)
{
    if (addr < 0)
    {
        std::cerr << "Runtime error: negative memory write"
                  << std::endl;
        std::exit(1);
    }
    if (addr < STACK_SIZE)
...
void handle_divide_by_zero()
{
    if (ex_stack.empty())
    {
        std::cerr << "Runtime error: division by zero, no handler"
                  << std::endl;
        std::exit(1);
    }
    std::cerr << "Exception caught: division by zero"
              << std::endl;
...
void run()
{
    ...
    while (PC < code.size())
    {
...
#ifndef NDEBUG
        std::cout << "CNT=" << std::left << std::setw(8) << cnt++
                  << "SP=" << std::left << std::setw(8) << SP
                  << "FP=" << std::left << std::setw(8) << FP
                  << "PC=" << std::left << std::setw(8) << PC
                  << "OP=" << std::left << std::setw(8) << op
                  << "VAL=" << std::left << std::setw(8) << val
                  << std::endl;
#endif
        ...
        else if (op == "SWAP")
        {
            if (SP < val + 1)
            {
                std::cerr << "Runtime error: stack underflow on SWAP"
                          << std::endl;
                std::exit(1);
            }
        ...
        else if (op == "I2F")
        {
            long long a = pop_stack();
            push_stack(
                std::bit_cast<long long>(static_cast<double>(a)));
            PC++;
        }
        else if (op == "F2I")
        {
        ...
        {
            long long a = pop_stack();
            push_stack(std::bit_cast<long long>(
        ...
        else if (op == "ADDSP")
        {
            ...
            {
                std::cerr << "Runtime error: invalid ADDSP result"
                          << std::endl;
                std::exit(1);
            }
            PC++;
        ...
        {
            long long size = pop_stack();
            int addr = allocator.alloc(size);
            push_stack(addr);
            PC++;
        }
        else if (op == "FREE")
        ...
        else if (op == "GET")
        {
            ...
            std::cin >> c;
            long long v =
                static_cast<long long>(static_cast<unsigned char>(c));
            push_stack(v);
            PC++;
        }
        else if (op == "GETS")
        ...
        {
            std::cerr << "Runtime error: unknown opcode \"" << op
                      << "\" at PC=" << PC << "" << std::endl;
            std::exit(1);
        }
    }
...
int main(int argc, char* argv[])
{
    if (argc < 2)
    {
        std::cerr << "Usage: " << argv[0] << " <bytecode.txt>"
                  << std::endl;
        return 1;
    }
